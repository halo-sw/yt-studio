...
### 대본·발행 리텐션 공식 (외부 벤치마크 수용분, 2026-07-29 — 기존 규칙과 충돌 시 이 문서 다른 장이 우선)
- **훅은 충격 숫자·반직관 문장으로 0초 시작** (기존 콜드오픈 규칙과 동일 — 인사·채널 소개 금지).
- **90초마다 패턴 인터럽트**: 씬 설계 때 명시적으로 배치 — 포맷 전환(카드↔실사), 시청자에게 질문, 톤·화자 변화 중 하나. 같은 리듬 90초 초과 금지.
- **신뢰 브리지 1회, 30초 이내**: 이 얘기를 믿어도 되는 근거 한 줄(심리='같이 공부' 프레임+이론 출처, 정보 트랙=공고·보도 원문). 길면 자랑이 된다.
- **엔딩 = 훅 콜백 + 행동 1개 + 구독 명분 1줄**. 예고·나열형 아웃트로 금지.
- **제목 60자 이내**, 고정댓글은 3안 만들어 1개 선택.
- **채널 판단은 20~30편 이후에**: 알고리즘이 시청자 매칭을 학습하는 데 20편+가 필요하다. 그 전의 낮은 조회로 니치 실패를 선언하지 않는다. 게이트(전략 문서)는 자원 "배분"만 조정하고 채널 중단은 30편 전 금지. **고정 슬롯(같은 요일·같은 시각) 6개월 엄수.**
- 소재 깊이 테스트: 시리즈 기획 시 **제목 52개를 뽑아본다** — 못 채우면 니치가 좁은 것 (channels/소재함/README.md).
